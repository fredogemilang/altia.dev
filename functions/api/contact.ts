import { handleContactSubmission, type ContactPayload, type EnvBindings } from '../../src/lib/api-handlers';
...
export async function onRequestPost(context: EventContext<EnvBindings, string, any>) {
  try {
    const payload = (await context.request.json().catch(() => ({}))) as ContactPayload;
    const result = await handleContactSubmission(payload, context.env);
